...
Now, we can train our final model based on the ideal parameters calculated with our cross validation (ideal values are maximum depth of 5 and number of estimators 50). Fortunately, our final model has a lower RMSE (about .585, which is approximately .1 lower than the baseline model RMSE). Although this model is still not very accurate, it's certainly an improvement from the previous.
